...
<div class="container-fluid">
    <div class="row justify-content-center">
        ...
        <div class="col-12 col-sm-11 col-lg-9">
            <p class="mb-5">{!! __(':%1 COMMON LANDS NETWORK :%2 es una plataforma participativa, por lo que tu papel es clave. Visibiliza tu trabajo, conoce el de otros y colabora. Registrándote podrás incluir tu contacto en el directorio, además de compartir casos demostrativos, noticias, eventos, y otros recursos como vídeos, documentos, etc.', ['%1' => '<strong>', '%2' => '</strong>']) !!}</p>

            @include('partials.notify')

            {!! Form::open(['url' => route('register'), 'method' => 'post', 'files' => true, 'id' => 'form', 'novalidate']) !!}
                <h2>{{ __('Nuevo registro') }}</h2>
                <div class="row">
                    <div class="col-12 form-group">
                        <input type="email" class="form-control{{ $errors->has('email') ? ' is-invalid' : '' }}" name="email" value="{{ old('email') }}" placeholder="{{ __('Introduce tu mail') }}*" required>
                    </div>
                    <div class="col-sm-6 form-group">
                        <div class="input-group">
                            <input type="password" class="form-control{{ $errors->has('password') ? ' is-invalid' : '' }}" name="password" placeholder="{{ __('Contraseña') }}*" required>
                    ...
                    <div class="col-sm-6 form-group">
                        ...
                        <div class="input-group">
                            <input type="password" class="form-control" name="password_confirmation" placeholder="{{ __('Repetir contraseña') }}*" required>
                            <div class="input-group-append toggle-password">
                                <span class="input-group-text"><img src="{{ asset('svg/icons/eye.svg') }}" width="30" height="10"></span>
                            </div>
                        </div>
                ...
                <div class="row">
                    ...
                    <div class="col-12 form-group">
                        {!! Form::text('name', null, ['class' => 'form-control' . ($errors->has('name') ? ' is-invalid' : ''), 'placeholder' => __('Nombre y/o acrónimo') . '*', 'required']) !!}
                    </div>
                    <div class="col-sm-8 form-group">
                        {!! Form::email('directory_email', null, ['class' => 'form-control' . ($errors->has('directory_email') ? ' is-invalid' : ''), 'placeholder' => __('Email') . '*', 'required']) !!}
                    </div>
                ...
                <div id="additional_info" style="{{ 1 == old('entity_id', ! empty($directory) ? $directory->entity_id : 0) ? 'display: none;' : '' }}">
                    ...
                    <div class="row">
                        ...
                        <div class="col-12 form-group">
                            {!! Form::number('surface', null, ['class' => 'form-control' . ($errors->has('surface') ? ' is-invalid' : ''), 'placeholder' => __('Superficie (Ha)'), 'min' => 0]) !!}
                        </div>
                    </div>
                </div>
                ...
                <div class="row mb-5">
                    <div class="col-12 form-group">
                        <div class="custom-control custom-checkbox">
                            {!! Form::checkbox('accept_lopd', 1, null, ['id' => 'accept_lopd', 'class' => 'custom-control-input' . ($errors->has('accept_lopd') ? ' is-invalid' : ''), 'required']) !!}
                            <label class="custom-control-label" for="accept_lopd">
                                {!! __('He leído y acepto la :%1 política de privacidad general :%2', ['%1' => '<a href="' . route('privacy_policy') . '" target="_blank">', '%2' => '</a>']) !!}
                            </label>
                        </div>
                    </div>
                    <div class="col-12 form-group">
                        <div class="custom-control custom-checkbox">
                            {!! Form::checkbox('terms', 1, null, ['id' => 'terms', 'class' => 'custom-control-input' . ($errors->has('terms') ? ' is-invalid' : ''), 'required']) !!}
                            <label class="custom-control-label" for="terms">
                                {!! __('Estoy de acuerdo con los :%1 términos y condiciones de uso :%2', ['%1' => '<a href="' . route('terms') . '" target="_blank">', '%2' => '</a>']) !!}
                            </label>
                        </div>
                    </div>
                    <div class="col-12 form-group">
                        <div class="custom-control custom-checkbox">
                            {!! Form::checkbox('accept_share', 1, null, ['id' => 'accept_share', 'class' => 'custom-control-input' . ($errors->has('accept_share') ? ' is-invalid' : ''), 'required']) !!}
                            <label class="custom-control-label" for="accept_share">
                                {!! __('Doy mi consentimiento explícito a que los datos publicados en la plataforma puedan ser difundidos de forma libre, por cualquier persona, entidad, empresa u asociación con el conocimiento de que existe transferencia internacional de datos, para ello he leído y acepto la :%1 política de privacidad del formulario de registro :%2', ['%1' => '<a href="#privacyModal" data-toggle="modal">', '%2' => '</a>']) !!}
                            </label>
                        </div>
                    </div>
                    {{-- <div class="col-12 form-group">
                ...
                <div class="form-group row">
                    <div class="col-12">
                        <input type="image" class="btn btn-xl mt-4" title="{{ __('Registro') }}" src="{{ url('svg/icons/send.svg') }}" value="{{ __('Enviar') }}">
                    </div>
                </div>
...
@include('page._partials.privacy-form.' . strtolower(locale(true)))
