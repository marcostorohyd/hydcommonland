...
<div class="row mb-4">
    <div class="col-12">
        <a href="{{ route('backend.news.create') }}" class="btn btn-dark btn-lg" title="{{ __('Nueva noticia') }}">{{ __('Nueva noticia') }}</a>
    </div>
</div>
<div class="row">
    <div class="col-lg-12">
...
@section('js')
<script src="{{ url('js/news.js') }}?{{ config('commonlandsnet.version') }}"></script>
<script type="text/javascript">
    $(function() {
        ...
        ops = {
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('backend.news.datatable') }}',
                method: 'POST',
                data: function (d) {
            ...
            columnDefs: [{
                ...
                class: 'text-center',
                render: function ( data, type, row ) {
                    @can('update-delete', \App\News::class)
                        var html = '<a class="mr-3" href="/backend/news/' + data + '/edit" title="{{__('Editar noticia') }}"><i class="icon icon-pencil icon-md"></i></a>';
                        html += '<a onclick="newsDestroy(' + data + ');return false;" href="#" title="{{__('Borrar noticia') }}"><i class="icon icon-trash icon-md"></i></a>';
                    @else
                        var html = '<a href="/backend/news/' + data + '" title="{{__('Ver noticia') }}"><i class="icon icon-pencil icon-md"></i></a>';
                    @endif
                    return html;
                },
            }],
