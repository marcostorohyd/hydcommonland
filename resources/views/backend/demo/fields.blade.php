@if (! empty($demo))
            ...
            @switch($demo->status_id)
                ...
                @case(\App\Status::REFUSED)
                    <div role="alert" class="alert alert-danger fade show">
                        ...
                        {{ __('Este caso demostrativo ha sido rechazado') }}
                    </div>
                ...
                @default
                    <div role="alert" class="alert alert-info fade show">
                        ...
                        {{ __('Este caso demostrativo está pendiente de aprobación') }}
                    </div>
    ...
    <div class="row">
        <div class="col-12 col-sm-6 form-group">
            @if (in_array($demo->status_id, [\App\Status::PENDING, \App\Status::REFUSED]))
                <a href="{{ route('backend.demo.approve', $demo->id) }}" class="btn btn-dark btn-lg mr-3" type="submit">{{ __('Aprobar') }}</a>
            @endif
            @if (in_array($demo->status_id, [\App\Status::PENDING, \App\Status::APPROVED]))
                <a href="{{ route('backend.demo.refuse', $demo->id) }}" class="btn btn-dark btn-lg" type="submit">{{ __('Rechazar') }}</a>
            @endif
        </div>
        @if (\App\Status::APPROVED == $demo->status_id)
            <div class="col-12 col-sm-6 form-group text-sm-right">
                <a href="{{ route('demo.show', $demo->id) }}" class="btn btn-dark btn-lg" type="submit">{{ __('Ver') }}</a>
            </div>
        @endif
    </div>
@endif

<h2>{{ __('Datos del caso demostrativo') }}</h2>
<div class="row">
    <div class="col-12 form-group">
        {!! Form::text('name', null, ['class' => 'form-control' . ($errors->has('name') ? ' is-invalid' : ''), 'placeholder' => __('Título') . '*', 'required']) !!}
    </div>
    <div class="col-12 form-group">
        {!! Form::text('date', old('date', ! empty($demo->date) ? $demo->date->format('Y-m-d') : ''), ['class' => 'form-control date' . ($errors->has('date') ? ' is-invalid' : ''), 'placeholder' => __('Fecha') . '*', 'required']) !!}
    </div>
    <div class="col-12 form-group">
        {!! Form::text('address', null, ['class' => 'form-control' . ($errors->has('address') ? ' is-invalid' : ''), 'placeholder' => __('Lugar'),]) !!}
    </div>
    <div class="col-12 form-group">
    ...
    <div class="col-12 form-group">
        <div class="input-group">
            {!! Form::select('values[]', $data['values'], null, ['class' => 'form-control' . ($errors->has('values') ? ' is-invalid' : ''), 'id' => 'values', 'required', 'multiple']) !!}
            <div class="input-group-append select-all">
                <img src="{{ asset('svg/icons/all.svg') }}" width="30" height="30">
            </div>
        </div>
    ...
    <div class="col-12 form-group">
        {!! Form::select('condition_id', $data['conditions'], null, ['class' => 'form-control' . ($errors->has('condition_id') ? ' is-invalid' : ''), 'placeholder' => '+ ' . __('Estatus') . '*', 'required']) !!}
    </div>
    <div class="col-12 form-group">
        {!! Form::email('email', null, ['class' => 'form-control' . ($errors->has('email') ? ' is-invalid' : ''), 'placeholder' => __('Contacto (email)')]) !!}
    </div>
    <div class="col-12 form-group">
        {!! Form::url('link', null, ['class' => 'form-control' . ($errors->has('link') ? ' is-invalid' : ''), 'placeholder' => __('Enlace') . ' 1']) !!}
    </div>
    <div class="col-12 form-group" data-condition="link !== ''">
        {!! Form::url('link2', null, ['class' => 'form-control' . ($errors->has('link2') ? ' is-invalid' : ''), 'placeholder' => __('Enlace') . ' 2']) !!}
    </div>
    <div class="col-12 form-group" data-condition="link2 !== ''">
        {!! Form::url('link3', null, ['class' => 'form-control' . ($errors->has('link3') ? ' is-invalid' : ''), 'placeholder' => __('Enlace') . ' 3']) !!}
    </div>
    <div class="col-12 form-group" data-condition="link3 !== ''">
        {!! Form::url('link4', null, ['class' => 'form-control' . ($errors->has('link4') ? ' is-invalid' : ''), 'placeholder' => __('Enlace') . ' 4']) !!}
    </div>
    <div class="col-12 form-group" data-condition="link4 !== ''">
        {!! Form::url('link5', null, ['class' => 'form-control' . ($errors->has('link5') ? ' is-invalid' : ''), 'placeholder' => __('Enlace') . ' 5']) !!}
    </div>
</div>
    ...
    <div class="col-12 form-group lang-group">
        ...
        @foreach (locales() as $lang)
            <div class="lang" lang="{{ $lang }}">
                {!! Form::textarea($lang . '[description]', old($lang . '.description', ! empty($demo) ? $demo->translate($lang)->description : ''), ['class' => 'form-control description' . ($errors->has($lang . '.description') ? ' is-invalid' : ''), 'placeholder' => __('Escribe algo...')]) !!}
            </div>
        @endforeach
    </div>
</div>

<h2>{{ __('Foto de portada') }}* <small>({{ \App\Media::humanize(\App\DemoCaseStudy::IMAGE_ALLOWED) }} {{ __('tamaño máx. :size MB.', ['size' => \App\DemoCaseStudy::IMAGE_MAX_SIZE / 1000]) }})</small></h2>
<div class="row">
    <div class="col-12 form-group">
        <div id="dropzone" class="dropzone {{ $errors->has('image') ? ' is-invalid' : '' }}">
            ...
            <div class="fallback">
                {!! Form::file('file', ['class' => 'form-control' . ($errors->has('image') ? ' is-invalid' : ''), 'accept' => \App\DemoCaseStudy::IMAGE_ALLOWED]) !!}
            </div>
        </div>
...
<script>
    ...
    $(function() {
        ...
        var myDropzone = new Dropzone('#dropzone', {
            acceptedFiles: @json(\App\DemoCaseStudy::IMAGE_ALLOWED),
            ...
            parallelUploads: 1,
            maxFiles: 1,
            maxFilesize: @json(\App\DemoCaseStudy::IMAGE_MAX_SIZE / 1000),
            addRemoveLinks: true,
            dictRemoveFile: '{{ __('Eliminar imagen') }}',
            dictFileTooBig: '{{ __('La imagen es mayor de :size MB', ['size' => \App\DemoCaseStudy::IMAGE_MAX_SIZE / 1000]) }}',
            timeout: 10000,
            thumbnailWidth: 1200,
            thumbnailHeight: 250,
            init: function () {
        ...
        // Fill image
        @if ($image = old('image', ! empty($demo->image)))
            var image = { name: '', size: ''};
            myDropzone.emit('addedfile', image);
            myDropzone.emit('thumbnail', image, '{{ old('image', (! empty($demo->image) ? $demo->imageUrl() : '')) }}');
