...
<div class="row mb-4">
    <div class="col-12">
        <a href="{{ route('backend.demo.create') }}" class="btn btn-dark btn-lg" title="{{ __('Nuevo caso demostrativo') }}">{{ __('Nuevo caso demostrativo') }}</a>
    </div>
</div>
<div class="row">
    <div class="col-lg-12">
...
@section('js')
<script src="{{ url('js/demo.js') }}?{{ config('commonlandsnet.version') }}"></script>
<script type="text/javascript">
    $(function() {
        ...
        ops = {
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('backend.demo.datatable') }}',
                method: 'POST',
                data: function (d) {
            ...
            columnDefs: [{
                ...
                class: 'text-center',
                render: function ( data, type, row ) {
                    @can('update-delete', \App\DemoCaseStudy::class)
                        var html = '<a class="mr-3" href="/backend/demo/' + data + '/edit" title="{{__('Editar caso demostrativo') }}"><i class="icon icon-pencil icon-md"></i></a>';
                        html += '<a onclick="demoDestroy(' + data + ');return false;" href="#" title="{{__('Borrar caso demostrativo') }}"><i class="icon icon-trash icon-md"></i></a>';
                    @else
                        var html = '<a href="/backend/demo/' + data + '" title="{{__('Ver caso demostrativo') }}"><i class="icon icon-pencil icon-md"></i></a>';
                    @endif
                    return html;
                },
            }],
