@if (! empty($event))
            ...
            @switch($event->status_id)
                @case(\App\Status::APPROVED)
                    <div role="alert" class="alert alert-success fade show">
                        ...
                        {{ __('Este evento ha sido aprobado') }}
                    </div>
    ...
    <div class="row">
        <div class="col-12 col-sm-6 form-group">
            @if (in_array($event->status_id, [\App\Status::PENDING, \App\Status::REFUSED]))
                <a href="{{ route('backend.event.approve', $event->id) }}" class="btn btn-dark btn-lg mr-3" type="submit">{{ __('Aprobar') }}</a>
            @endif
            @if (in_array($event->status_id, [\App\Status::PENDING, \App\Status::APPROVED]))
                <a href="{{ route('backend.event.refuse', $event->id) }}" class="btn btn-dark btn-lg" type="submit">{{ __('Rechazar') }}</a>
            @endif
        </div>
        @if (\App\Status::APPROVED == $event->status_id)
            <div class="col-12 col-sm-6 form-group text-sm-right">
                <a href="{{ route('event.show', $event->id) }}" class="btn btn-dark btn-lg" type="submit">{{ __('Ver') }}</a>
            </div>
        @endif
    </div>
@endif
...
<div class="row">
    <div class="col-12 form-group">
        {!! Form::text('name', null, ['class' => 'form-control' . ($errors->has('name') ? ' is-invalid' : ''), 'placeholder' => __('Nombre del evento') . '*', 'required']) !!}
    </div>
    <div class="col-12 form-group">
        {!! Form::email('email', null, ['class' => 'form-control' . ($errors->has('email') ? ' is-invalid' : ''), 'placeholder' => __('Organizador (email)') . '*', 'required']) !!}
    </div>
    <div class="col-sm-6 form-group">
        {!! Form::text('start', old('start', ! empty($event->start) ? $event->start->format('Y-m-d') : ''), ['class' => 'form-control date' . ($errors->has('start') ? ' is-invalid' : ''), 'placeholder' => __('Fecha de inicio') . '*', 'required']) !!}
    </div>
    <div class="col-sm-6 form-group">
        {!! Form::text('end', old('end', ! empty($event->end) ? $event->end->format('Y-m-d') : ''), ['class' => 'form-control date' . ($errors->has('end') ? ' is-invalid' : ''), 'placeholder' => __('Fecha de fin')]) !!}
    </div>
    <div class="col-12 form-group">
        {!! Form::url('register_url', null, ['class' => 'form-control' . ($errors->has('register_url') ? ' is-invalid' : ''), 'placeholder' => __('URL de registro')]) !!}
    </div>
    <div class="col-12 form-group">
        {!! Form::select('assistance_id', $data['assistances'], null, ['class' => 'form-control' . ($errors->has('assistance_id') ? ' is-invalid' : ''), 'placeholder' => '+ ' . __('Asistencia')]) !!}
    </div>
    <div class="col-12 form-group">
        {!! Form::select('type_id', $data['types'], null, ['class' => 'form-control' . ($errors->has('type_id') ? ' is-invalid' : ''), 'placeholder' => '+ ' . __('Tipo del evento') . '*', 'required']) !!}
    </div>
    <div class="col-12 form-group">
    ...
    <div class="col-12 form-group">
        {!! Form::text('language', null, ['class' => 'form-control' . ($errors->has('language') ? ' is-invalid' : ''), 'placeholder' => __('Idioma del evento')]) !!}
    </div>
    <div class="col-12 form-group">
        {!! Form::text('venue_name', null, ['class' => 'form-control' . ($errors->has('venue_name') ? ' is-invalid' : ''), 'placeholder' => __('Nombre del lugar')]) !!}
    </div>
    <div class="col-12 form-group">
        {!! Form::text('venue_address', null, ['class' => 'form-control' . ($errors->has('venue_address') ? ' is-invalid' : ''), 'placeholder' => __('Dirección del lugar')]) !!}
    </div>
    <div class="col-12 form-group">
...
<h2>{{ __('Imagen') }}* <small>({{ \App\Media::humanize(\App\Event::IMAGE_ALLOWED) }} {{ __('tamaño máx. :size MB.', ['size' => \App\Event::IMAGE_MAX_SIZE / 1000]) }})</small></h2>
<div class="row">
    <div class="col-12 form-group">
        <div id="dropzone" class="dropzone {{ $errors->has('image') ? ' is-invalid' : '' }}">
            ...
            <div class="fallback">
                {!! Form::file('file', ['class' => 'form-control' . ($errors->has('image') ? ' is-invalid' : ''), 'accept' => \App\Event::IMAGE_ALLOWED]) !!}
            </div>
        </div>
...
<script>
    ...
    $(function() {
        ...
        var myDropzone = new Dropzone('#dropzone', {
            acceptedFiles: @json(\App\Event::IMAGE_ALLOWED),
            ...
            parallelUploads: 1,
            maxFiles: 1,
            maxFilesize: @json(\App\Event::IMAGE_MAX_SIZE / 1000),
            addRemoveLinks: true,
            dictRemoveFile: '{{ __('Eliminar imagen') }}',
            dictFileTooBig: '{{ __('La imagen es mayor de :size MB', ['size' => \App\Event::IMAGE_MAX_SIZE / 1000]) }}',
            timeout: 10000,
            thumbnailWidth: 1200,
            thumbnailHeight: 250,
            init: function () {
        ...
        // Fill image
        @if ($image = old('image', ! empty($event->image)))
            var image = { name: '', size: ''};
            myDropzone.emit('addedfile', image);
            myDropzone.emit('thumbnail', image, '{{ old('image', (! empty($event->image) ? $event->imageUrl() : '')) }}');
