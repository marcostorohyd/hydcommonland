...
<div class="row mb-4">
    <div class="col-12">
        <a href="{{ route('backend.media.create') }}" class="btn btn-dark btn-lg" title="{{ __('Nuevo archivo multimedia') }}">{{ __('Nuevo archivo multimedia') }}</a>
    </div>
</div>
<div class="row">
    <div class="col-lg-12">
            ...
            <table id="datatable-filter" class="table table-hover datatables m-t">
                ...
                <thead>
                    ...
                    <tr>
                        ...
                        <th>
                            {!! Form::select('format_id', $data['formats'], null, ['class' => 'form-control', 'placeholder' => __('- Todos -')]) !!}
                        </th>
                        <th>
...
@section('js')
<script src="{{ url('js/media.js') }}?{{ config('commonlandsnet.version') }}"></script>
<script type="text/javascript">
    $(function() {
        ...
        ops = {
            ...
            serverSide: true,
            ajax: {
                url: '{{ route('backend.media.datatable') }}',
                method: 'POST',
                data: function (d) {
            ...
            columnDefs: [{
                ...
                class: 'text-center',
                render: function ( data, type, row ) {
                    @can('update-delete', \App\MediaLibrary::class)
                        var html = '<a class="mr-3" href="/backend/media/' + data + '/edit" title="{{__('Editar mediateca') }}"><i class="icon icon-pencil icon-md"></i></a>';
                        html += '<a onclick="mediaDestroy(' + data + ');return false;" href="#" title="{{__('Borrar mediateca') }}"><i class="icon icon-trash icon-md"></i></a>';
                    @else
                        var html = '<a href="/backend/media/' + data + '" title="{{__('Ver mediateca') }}"><i class="icon icon-pencil icon-md"></i></a>';
                    @endif
                    return html;
                },
            }],
