...
@section('title', __('Solicitud cambio de datos de contacto general'))

@section('content')

    <div class="row mb-4">
        <div class="col-12 col-sm-auto">
            <a href="{{ route('backend.directory-change.approve', $directoryChange->id) }}" class="btn btn-dark btn-lg" title="{{ __('Aprobar cambios') }}">{{ __('Aprobar cambios') }}</a>
        </div>
        <div class="col-12 col-sm-auto">
            <a href="{{ route('backend.directory-change.refuse', $directoryChange->id) }}" class="btn btn-dark btn-lg" title="{{ __('Rechazar cambios') }}">{{ __('Rechazar cambios') }}</a>
        </div>
    </div>

    {!! Form::model($directoryChange) !!}
        ...
        <div id="person_contact" style="{{ 1 == $directoryChange->entity_id ? 'display: none;' : '' }}">
        ...
        <div id="additional_info" style="{{ 1 == $directoryChange->entity_id ? 'display: none;' : '' }}">
            ...
            <div class="col-12 form-group lang-group">
                ...
                @foreach (locales() as $lang)
                    <div class="lang" lang="{{ $lang }}">
                        {!! Form::textarea($lang . '[description]', old($lang . '.description', ! empty($directoryChange) ? $directoryChange->translate($lang)->description : ''), ['class' => 'form-control description', 'placeholder' => __('Escribe algo...')]) !!}
                    </div>
                @endforeach
            </div>
        </div>
            ...
            <div class="col-12 form-group">
                <div class="dropzone">
                    <div class="dz-preview">
                        <div class="dz-image">
                            <img src="{{ $directoryChange->imageUrl() }}" alt="">
                        </div>
                    </div>
                </div>
            </div>
